...
class Cds(object):
    ...
    def amino_acid(self, seq, position, nt_ref, nt_alt):
        # Should return the reference amino-acid, the alternative amino-acid,
        # the reference codon and the alternative codon
        ...
        #position chromosomique dans le fasta
...
#            
        ...
        alt_codon[reste] = nt_alt #on passe de tuple (plus dur a manipuler) à list
        
        if nt_ref != ref_codon[reste]:
            #on vérifie que fasta (à droite) et vcf (à gauche) sont bien d'accord
            #les crochets permettent d'obtenir la position
            return "ERROR FASTA VS REF", "", "", ""
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--fasta', required=False, default="Homo_sapiens.GRCh38.all_cds.fa", type=str,
                        dest="f", metavar="<fasta>",
                        help="The relative name of the .fasta file")
    parser.add_argument('-g', '--gtf', required=False, default="GRCh38.chr.gtf", type=str,
                        dest="g", metavar="<gtf>",
                        help="The relative name of the .gtf file")
    parser.add_argument('-v', '--vcf', required=False, default="filtered_Chr20.vcf", type=str,
                        dest="v", metavar="<vcf>",
        ...
        if line[0] != '#':
            ...
            if len(split_line[3]) == 1 and len(split_line[4]) == 1 and split_line[4] != ".":
                ...
                for tr_id in split_line[-1].replace('\n', '').replace('; ', ', ').split(","):
                    ...
                    elif ref_aa == 'ERROR FASTA VS REF':
