...
def bootstrap(nbr_snps, array, bootstrap_resample=1000):
    ...
    Ratio=[]
    for j in range (bootstrap_resample):
        X=[]
        repet=[]
        i=0
        while i<nbr_snps:
            a = randint(0, array.shape[0]-1) #si lignes sont des SNPs
            if a not in repet:
                X+=[array[a,:]]
                repet+=[a]              #tjrs des SNPs différents
                i+=1
        Ratio+=[epistasis(np.array(X))]  
    assert(len(Ratio)==bootstrap_resample)
    print(Ratio)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--stop', required=False, default ="filtered_Chr20.Stop.vcf", type=str,
                        dest="stop", metavar="<vcf>",
                        help="The relative name of the stop .vcf file")
    parser.add_argument('-s', '--syn', required=False, default ="filtered_Chr20.Syn.vcf", type=str,
                        dest="syn", metavar="<vcf>",
                        help="The relative name of the synonymous .vcf file")
    parser.add_argument('-n', '--nonsyn', required=False, default ="filtered_Chr20.NonSyn.vcf", type=str,
                        dest="non_syn", metavar="<vcf>",
                        help="The relative name of the non-synonymous .vcf file")
    parser.add_argument('-p', '--panel', required=False, default ="popID.panel", type=str,
